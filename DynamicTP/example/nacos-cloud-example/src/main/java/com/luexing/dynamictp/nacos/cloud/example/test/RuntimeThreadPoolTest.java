 ...
 * 掠星 DynamicTP 动态线程池
 */
package com.luexing.dynamictp.nacos.cloud.example.test;
...
public class RuntimeThreadPoolTest {
    ...
    private final ThreadPoolExecutor luexing2;
    ...
    public void init() {
        ...
        // 启动定时高压段调度
        schedulePeriodicBurstTask(luexing1, "luexing-1");
        schedulePeriodicBurstTask(luexing2, "luexing-2");
    }
    ...
    private void simulateHighActiveThreadUsage() {
        for (int i = 0; i < MAX_TASK; i++) {
            sleepRandom(1, 1000);
            try {
                luexing1.execute(() -> sleepRandom(200, 1000));
    ...
    private void simulateQueueUsageHigh() {
        for (int i = 0; i < MAX_TASK; i++) {
            sleepRandom(100, 500);
            try {
                luexing2.execute(() -> sleepRandom(200, 500));
            } catch (Exception ignored) {
            }
        }
    }
